ad_numbers = ""

ad_numbers_to_names = ""

agent_config = ""

allow_short_container_image_names = false

apiserver_private_host = ""

assign_dns = ""

assign_public_ip = ""

block_volume_type = "paravirtualized"

capacity_reservation_id = null

cloud_init = []

cluster_ca_cert = null

cluster_dns = null

cluster_id = null

cluster_type = "basic"

cni_type = "flannel"

compartment_id = null

compute_clusters = {}

defined_tags = {}

disable_default_cloud_init = false

freeform_tags = {}

gmc_scale_is_downsize_enabled = true

gmc_scale_is_upsize_enabled = true

gmc_scale_target_size = 18

image_id = null

image_ids = {}

image_os = "Oracle Linux"

image_os_version = "8"

image_type = "oke"

indexed_images = {}

kubeproxy_mode = "iptables"

kubernetes_version = "v1.26.2"

legacy_imds_endpoints_disabled = false

max_pods_per_node = 31

node_labels = {}

node_metadata = {}

nsg_ids = {}

oke_ip_families = ["IPv4"]

platform_config = null

pod_nsg_ids = []

pod_subnet_id = ""

preemptible_config = { "enable" : false, "is_preserve_boot_volume" : false }

pv_transit_encryption = false

shape = { "boot_volume_size" : 50, "memory" : 16, "ocpus" : 2, "shape" : "VM.Standard.E4.Flex" }

ssh_public_key = null

state_id = null

subnet_ids = {}

tag_namespace = "oke"

tenancy_id = null

timezone = ""

use_defined_tags = false

volume_kms_key_id = null

worker_nsg_ids = []

worker_pool_mode = "node-pool"

worker_pool_size = 0

worker_pools = {}

worker_subnet_id = ""
